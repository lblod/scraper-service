...
* `STORE_ALL_PAGES`: (default: `false`) when disabled (`false`) will only store pages containing Notulen, Agenda, Besluitenlijst, Uittreksel, Besluit or BehandelingVanAgendapunt. (using the same heuristic as incremantal retrieval).
* `ENABLE_FILTERING_ON_INTERESTING_PROPERTIES`: (default: `true`) when disabled (`false`) will not look at `INTERESTING_PROPERTIES` to decide if the page should be kept.
* `INTERESTING_PROPERTIES`: (default: `heeftNotulen,heeftAgenda,heeftBesluitenlijst,heeftUittreksel,linkToPublication`) list of properties to find in a page to decide if we should keep it. At least one property must be in the page.
